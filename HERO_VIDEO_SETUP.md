...
حرکت اسکرول همچنان زمان ویدئو را با پیشرفت سکشن هماهنگ می‌کند. نسخه موبایل با عرض ۷۲۰ پیکسل و فشرده‌سازی جداگانه استفاده می‌شود تا حجم دانلود روی تلفن کاهش یابد. خود ویدئو داخل قاب داخلی `.hero-video-frame` قرار دارد و از لبه‌های Hero فاصله دارد؛ در دسکتاپ و موبایل این فاصله با کلاس‌های responsive تنظیم می‌شود و گوشه‌های گرد و حاشیه ظریف به قاب اضافه شده است. برای تغییر اندازه قاب، مقدار `inset-3` و `sm:inset-6` در `HeroSlider.tsx` را تغییر دهید.
