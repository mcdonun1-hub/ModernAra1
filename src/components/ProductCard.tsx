...
export default function ProductCard({ product, onView }: ProductCardProps) {
  ...
  return (
    ...
    >
      {/* Image / 360-style product motion */}
      <div className="group-360 relative aspect-[3/4] overflow-hidden bg-dark-50" aria-label={`نمای ۳۶۰ درجه ${product.name}`}>
        <div className="product-360-stage relative h-full w-full">
          <img src={asset(product.image_url)} alt={product.name} className="product-360-face h-full w-full object-cover" />
          <img src={asset(product.image_url)} alt="" aria-hidden="true" className="product-360-face absolute inset-0 h-full w-full object-cover" style={{ transform: 'rotateY(180deg)' }} />
        </div>
        <div className="product-360-glint pointer-events-none absolute inset-y-0 -left-1/2 w-1/3 bg-gradient-to-r from-transparent via-white/35 to-transparent" aria-hidden="true" />
